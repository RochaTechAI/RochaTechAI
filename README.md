<div align="center">

# Uanderson Evangelista Rocha
*Desenvolvedor Backend (Java & PHP) | Graduando em ADS e Pós-Graduando em Engenharia de Software*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/uandersonrocha)
[![E-mail](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](urochatech@gmail.com)

</div>

## 📑 Resumo Profissional
Profissional em transição estratégica da área da Saúde para a Tecnologia. Utilizo o rigor analítico da formação em Radiologia para o desenvolvimento de software robusto. Atualmente focado em ecossistemas *Java (Spring Boot)* e *PHP (laravel), com aplicação de **Clean Code, **Arquitetura MVC* e *Docker*.

* 💡 *Foco Atual:* Especialização em AWS Cloud pela Escola da Nuvem.
* 🎯 *Objetivo:* Estágio em Desenvolvimento de Software / Backend.

---

## 🛠️ Stack Técnica e Competências
<table align="center">
  <tr>
    <td align="center" width="250"><strong>Linguagens & Frameworks</strong></td>
    <td align="center" width="250"><strong>Infraestrutura & DevOps</strong></td>
    <td align="center" width="250"><strong>Arquitetura & Bancos</strong></td>
  </tr>
  <tr>
    <td>
      • Java (Spring Boot)<br>
      • PHP 8.2<br>
      • Maven
    </td>
    <td>
      • Docker & Docker Compose<br>
      • AWS (Em formação)<br>
      • Git & GitHub
    </td>
    <td>
      • MySQL 8.0<br>
      • APIs RESTful<br>
      • Design Patterns (Singleton)
    </td>
  </tr>
</table>

---

## 🔬 Projetos em Destaque

### [MedSaaS - Gestão de Clínicas Médicas](https://github.com/RochaTechAl/saas-agendamento-php)
* *Contexto:* Sistema Multi-tenant focado em isolamento seguro de dados.
* *Técnico:* PHP 8.2, MySQL e Docker com foco em Engenharia de Software.

### [Leitor de Imagens DICOM com IA](https://github.com/RochaTechAl/ia-radiologia)
* *Contexto:* Aplicação Java para extração de metadados e análise de imagens médicas (DICOM).
* *Técnico:* Processamento de imagens e estrutura preparada para modelos de IA.

---

## 🎓 Formação Acadêmica
* *Pós-graduação em Engenharia de Software* – Faculdade Metropolitana (Conclusão: 02/2026).
* *Análise e Desenvolvimento de Sistemas* – Faculdade das Américas (Previsão: 12/2027).
* *Tecnologia em Radiologia* – Faculdade de Medicina do ABC.

---

## 📈 Atividade no GitHub
<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=RochaTechAI&show_icons=true&theme=nord&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=RochaTechAI&layout=compact&langs_count=7&theme=nord"/>
</div>
